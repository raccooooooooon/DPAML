classdef MLPNet < handle
    ...
    methods
        ...
        function obj = AddInputLayer(obj,n_features,bias)
            ...
            obj.NL = obj.NL+1;
            obj.H{1} = ones(n_features+logical(bias),1);
            obj.Layers{1} = n_features+logical(bias);
            obj.net_bias{1} = logical(bias);
        end        
        function obj = AddHiddenLayer(obj,N_n,activation_function,bias)
            ...
            obj.NL = obj.NL+1;
            obj.Layers{obj.NL} = N_n+logical(bias);
            obj.H{obj.NL} = ones(N_n+logical(bias),1);
            obj.net_bias{obj.NL} = logical(bias);
            obj.Act_Fun{obj.NL-1} = activation_function;
            obj.Nets{obj.NL-1} = ones(obj.Layers{obj.NL-1}+logical(bias),1);
        ...
        function obj = AddOutputLayer(obj,n_output_features,activation_function,bias)
            ...
            % Update network features
            obj.NL = obj.NL+1;
            obj.Layers{obj.NL} = n_output_features+logical(bias);
            obj.H{obj.NL} = ones(n_output_features+logical(bias),1);
            obj.net_bias{obj.NL} = logical(bias);
            obj.Act_Fun{obj.NL-1} = activation_function;
        ...
        %%%%%%% Question 2. Fill the formula of crossentropy.%%%%%%%%%%%
            switch obj.lossfun
%                 case 'rmse'
%                     loss = 0.5*(sum(sum(target-Y_hat,1))).^2;
                case 'crossentropy'
                      loss = sum(sum(-target.*log(Y_hat),2))    ;                            % Use Y_hat and target to complete the formula.
%                 case 'binarycrossentropy'
%                     loss = -sum(sum(target.*log(Y_hat)+(1-target).*log(1-Y_hat),1));
        ...
        %%%%%%% Question 3. Fill the formula of momentum.%%%%%%%%%%%
            switch obj.momentum
                case 'regular'
                                                                         % Use obj.mu, obj.v{layer}, obj.rate, and delta_w to complete the formula.
                                                                               % Use obj.Weights{layer} and obj.v{layer} to complete the formula.  
                case 'nesterov'
